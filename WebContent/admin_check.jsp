			...
			<div class="container-fluid">
				...
				<form class="form-inline col-xs-10" action="${pageContext.servletContext.contextPath}/NewsServlet" method="get">
					...
					<div class="form-group">
						...
						<label>作者：</label>
						<input name="uname" type="text" class="form-control" placeholder="请输入作者" />
						<label>主题：</label>
				...
				<c:choose>
					...
					<div style="padding-top:20px">
		                ...
		                <div class="panel-heading">
		                	<h4 class="panel-title"><strong>待审核信息列表</strong></h4>
		                </div>  
						...
						<table class="table table-striped table-bordered" style="text-align:center;">					
							...
							<c:forEach items = "${pageBean.beanList}" var="news" varStatus="status">							
								<tr>
									...
									<td>${news.uname}</td>
									<c:choose>
										<c:when test="${news.checkStatus==3}"><td><span class="label label-success">审核通过</span></td></c:when>
										<c:when test="${news.checkStatus==2}"><td><span class="label label-primary">待审核</span></td></c:when>
										<c:otherwise><td><span class="label label-danger">审核未通过</span></td></c:otherwise>
									</c:choose>
									...
									<td>
										<button class="btn btn-warning btn-xs" onclick="{javascrtpt:window.location.href =
											'${pageContext.servletContext.contextPath}/NewsServlet?method=checkNews&nid=${news.nid}'}">审核</button>
										<button class="btn btn-danger btn-xs" onclick="{if(confirm('确认删除?')){javascrtpt:window.location.href = 
